...
class SpeechModel:
    ...
    def download_model(self):
        """
        Downloads the model checkpoint from HuggingFace.
        """
        checkpoint_dir = self.args.checkpoint_dir
        from huggingface_hub import snapshot_download
        if not os.path.exists(checkpoint_dir):
            os.makedirs(checkpoint_dir)
        print(f'downloading checkpoint for {self.name}')
        try:
            snapshot_download(repo_id=f'alibabasglab/{self.name}', local_dir=checkpoint_dir)
            return True
        except:
            return False
    ...
    def load_model(self):
        ...
        """
        checkpoint_path = os.path.join('models/ClearerVoice/checkpoints', self.name, 'last_best_checkpoint.pt')
