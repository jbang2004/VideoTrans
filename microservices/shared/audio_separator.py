...
class AudioSeparator(ABC):
    """音频分离器接口"""
    @abstractmethod
    def separate_audio(self, input_path: str, **kwargs) -> Tuple[np.ndarray, np.ndarray, int]:
        pass

class ClearVoiceSeparator(AudioSeparator):
    """
    使用 ClearVoice 实现音频分离，
    参考 backend/core/audio_separator.py 实现。
    """
    def __init__(self, model_name: str = 'MossFormer2_SE_48K'):
        self.model_name = model_name
        self.clearvoice = ClearVoice(task='speech_enhancement', model_names=[model_name])
